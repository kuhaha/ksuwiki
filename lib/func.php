...
///////////////////////////////
// copy recursively
function copy_r($dir_name, $new_dir){
  if (is_file($dir_name)) {
      return copy($dir_name, $new_dir);
  }
  if (!is_dir($new_dir)) {
     mkdir($new_dir);
  }
    ...
    if ($dh = opendir($dir_name)) {
      while (($file = readdir($dh)) !== false) {
        ...
        if (is_dir($dir_name . "/" . $file)) {
          copy_r($dir_name . "/" . $file, $new_dir . "/" . $file);
        }
        else {
...
EOD;
	...
	if (defined(SITE_SKIN_FILE)){
		$skin = SITE_SKIN_FILE;
	}else if(defined('SKIN_FILE')){
		$skin = SKIN_FILE;
	}
	if (isset($skin) && file_exists($skin) && is_readable($skin)){
