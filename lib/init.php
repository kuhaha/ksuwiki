...
define('S_COPYRIGHT',
'<strong>Pukiwiki with multiple site support,based on PukiWiki ' . S_VERSION . '</strong>' .
'<br/><strong>Faculty of Information Science, Kyushu Sangyo University</strong><br> '

//	' Copyright &copy; 2001-2006. ' .
//	' <a href="http://pukiwiki.sourceforge.jp/">PukiWiki Developers Team</a>.' .
//' License is GPL, http://www.gnu.org/licenses/gpl.html'
...
define('LANG_FILE_HINT', PKWK_LANG_DIR . LANG . '.lng.php');	// For encoding hint
define('LANG_FILE',      PKWK_LANG_DIR . UI_LANG . '.lng.php');	// For UI resource
$die = '';
...
define('UA_INI_FILE', PKWK_CONF_DIR . UA_PROFILE . '.ini.php');
...
$die = '';
foreach(array('DATA_DIR', 'DIFF_DIR', 'BACKUP_DIR', 'CACHE_DIR', 'UPLOAD_DIR', 'COUNTER_DIR', 'TRACKBACK_DIR') as $dir){
	if (! is_writable(constant($dir))){
		$die = 'Directory not found or not writable (' . constant($dir) . ')' . "\n";
		die_message($die);
	}
}
...
foreach (explode('&', $arg) as $key_and_value) {
	if (preg_match('/^([^=]+)=(.+)/', $key_and_value, $matches)){
		if(  mb_detect_encoding($matches[2]) != 'ASCII') {
			$_GET[$matches[1]] = $matches[2];
		}
	}else if (!isset($_GET['page'])){
		$g = rawurldecode($key_and_value);
		$g = strip_bracket($g);
		$g = input_filter($g);
		$_GET['page'] = $g;
	}
...
unset($matches);
$arg = preg_replace('/^([^=]+)=(.+)/', '',$arg);  //remove key=value pairs from $arg
/////////////////////////////////////////////////
// GET, POST, COOKIE
$get    = & $_GET;
$post   = & $_POST;
...
if (isset($vars['site'])){
	$site =  $vars['site'];
	if (! pkwksite_config( $site ) ){
		die_message('<h3 style="color:red;"> Site "' .$site. '"  does not exist!</h3>');
	}
	if (!pkwksite_accessible($site)){ // access restriction
		sleep(2);
		die('<h3 style="color:red;">You are not allowed accees to this site!</h3>');
	}
}

 if (isset( $vars['site_config']['title']))
	$page_title = $vars['site_config']['title'];
...
/////////////////////////////////////////////////
// 初期設定(ユーザ定義ルール読み込み)
require(PKWK_CONF_DIR . 'rules.ini.php');
