名前
    KsuWiki - 複数Wikiサイトを構築できるWikiエンジン、Based on
...
概要
    KsuWikiは、PukiWiki(ぷきうぃき)と同じく、ハイパーテキストを素早く自由に
    追加・削除・編集できるWebアプリケーション(Wiki, WikiWikiWeb)の一つです。
    PukiWikiはグループ間の知識の共有に用いることができますが、個人が自分のた
    めのアイデア帖や日記ツールとして活用することも可能です。また、管理者に
    だけ「簡単に編集できる」利点を残したまま、閲覧者に対して「編集のできない
    Webサイト」として情報を提供する構成も可能です。

    KsuWikiは、PukiWikiをベースにして再開発されたWikiエンジンである。複数の
    ひとつのWikiエンジンで、複数のWikiサイトを作成し、それぞれのサイトでは
    独自のスキン、独自のコンテンツ（Wikiページ、アクセス履歴、編集履歴）を
    持ちます。
   

サイトの作成、編集、削除、コピー
    サイト管理者は作成、編集、削除、コピーを行うことができます。サイトの設定
    はファイル型のデータベース（SQLite）によって管理されているので、サイトの
    設定を手軽に行えます。具体的に、以下のプラグインを使います。
   1> サイト一覧
   	cmd=sites
    #一覧画面から各管理作業は行える
   2> サイト作成
    cmd=sites&act=create
   3> サイト編集
    cmd=sites&act=modify&site_id=<id>
   4> サイト削除
    cmd=sites&act=delete&site_id=<id>
   5> サイトコピー
    cmd=sites&act=copy&site_id=<id>
   6> パスワード変更
    cmd=sites&act=passwd&site_id=<id>
